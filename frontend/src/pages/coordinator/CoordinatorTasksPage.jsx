      ...
      {activeTab === 'active' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {tasks.map((task) => {
            ...
            return (
              <div key={task.id} className="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 rounded-2xl border border-slate-100 flex flex-col justify-between hover:border-slate-200 transition space-y-4">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-slate-600 pt-3 border-t border-slate-100">
                    <div className="flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-[#7c3aed]" /> Deadline: <span className="font-bold text-slate-900">{task.deadline}</span>
                    </div>
          ...
          <div className="bg-white border border-slate-200 shadow-sm rounded-3xl max-w-3xl w-full p-6 lg:p-8 rounded-3xl border border-purple-200 shadow-2xl relative max-h-[85vh] flex flex-col">
            ...
            {loadingSubmissions ? (
              ...
              <div className="overflow-y-auto pr-1 space-y-3 flex-1">
                {submissions.map((sub) => (
                  <div key={sub.id} className="bg-white border border-slate-200 shadow-sm rounded-2xl p-4 rounded-xl border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 self-end md:self-auto shrink-0">
                      ...
                      {sub.status === 'PENDING' && <span className="text-xs text-slate-500 font-mono">Not Submitted Yet</span>}
                    </div>
                  </div>
                ))}
              </div>
